...
public class KafkaConfig {
  ...
  public NewTopic tradeEventsTopic() {
    return TopicBuilder.name(tradeEventsTopic).partitions(3).replicas(1).build();
  }

  @Bean
  public NewTopic tradeValidationTopic() {
    return TopicBuilder.name(tradeValidationTopic).partitions(2).replicas(1).build();
  }

  @Bean
  public NewTopic tradeExecutionTopic() {
    return TopicBuilder.name(tradeExecutionTopic).partitions(3).replicas(1).build();
  }

  @Bean
  public NewTopic tradeErrorsTopic() {
    return TopicBuilder.name(tradeErrorsTopic).partitions(2).replicas(1).build();
  }

  @Bean
  public NewTopic marketDataTopic() {
    return TopicBuilder.name(marketDataTopic).partitions(5).replicas(1).build();
  }
}
