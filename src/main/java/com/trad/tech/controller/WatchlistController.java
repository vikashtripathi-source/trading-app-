...
public class WatchlistController {
  ...
  public ResponseEntity<ApiResponse<Watchlist>> createWatchlist(
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            new ApiResponse<>(
                201, "Watchlist created successfully", createdWatchlist, "/api/watchlists"));
  }

  @Operation(
      summary = "Get watchlist by ID",
      description = "Retrieves a specific watchlist by its ID")
  @ApiResponses(
      value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<Watchlist>> getWatchlistById(
    ...
    return ResponseEntity.ok(
        new ApiResponse<>(
            200, "Watchlist retrieved successfully", watchlist, "/api/watchlists/" + id));
  }

  @Operation(
      summary = "Add symbol to watchlist",
      description = "Adds a stock symbol to the watchlist")
  @ApiResponses(
      value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<Watchlist>> addSymbolToWatchlist(
    ...
    if (!currentUserId.equals(watchlist.getUserId())) {
      return ResponseEntity.status(HttpStatus.FORBIDDEN)
          .body(
              new ApiResponse<>(403, "Access denied", null, "/api/watchlists/" + id + "/symbols"));
    }
  ...
  @Operation(
      ...
      description = "Removes a stock symbol from the watchlist")
  @ApiResponses(
      value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<Watchlist>> removeSymbolFromWatchlist(
    ...
    if (!currentUserId.equals(watchlist.getUserId())) {
      return ResponseEntity.status(HttpStatus.FORBIDDEN)
          .body(
              new ApiResponse<>(
                  403, "Access denied", null, "/api/watchlists/" + id + "/symbols/" + symbol));
    }
